...
const onApiFetchInitiateReducer = (
...
const onDataFetchFailureReducer = (
  employeeListState: IEmployeeListState,
) => {
  employeeListState.isLoading = false;
};

const onCreateNewApiStartReducer = (
  employeeListState: IEmployeeListState,
) => {
...
const onCreateNewApiFailReducer = (
  ...
  ToastAndroid.show(payload?.error || payload?.message || 'Something Went Wrong', ToastAndroid.SHORT)
  employeeListState.newEmployeeData.isLoading = false
...
const onUpdateEmployeeApiSuccessReducer = (
 ...
 log('employeeListState.list.data before : ', employeeListState.list.data, newUser)
...
const onUpdateEmployeeApiFailReducer = (
  ...
  ToastAndroid.show(payload?.error || payload?.message || payload?.data?.error || payload?.data?.message || 'Something Went Wrong', ToastAndroid.SHORT)
  employeeListState.updateEmployeeData.isLoading = false
...
const onDeleteEmployeeApiSuccessReducer = (
  ...
  const newUser = formatEmployeeData(data)
  const newEmployeeList = employeeListState.list?.data?.filter((item) => item._id !== newUser._id)
...
const onDeleteEmployeeApiFailReducer = (
  ...
  ToastAndroid.show(payload?.data?.error || payload?.data?.message  || 'Something Went Wrong', ToastAndroid.SHORT)
...
const getEmployeeListLoadingStatus = () => createSelector(
  (store: RootState) => store[EMPLOYEE_SLICE],
  (state: IEmployeeListState) => {
